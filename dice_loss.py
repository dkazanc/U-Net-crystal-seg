...
def compute_per_channel_dice(input, target, epsilon=1e-6, weight=None):
    ...
    input = F.one_hot(torch.squeeze(input), 4).permute((0, 3, 1, 2))
    target = F.one_hot(torch.squeeze(target), 4).permute((0, 3, 1, 2))
    ...
    res = 2 * (intersect / denominator.clamp(min=epsilon))
